      ...
      return (
        <React.Fragment key={label}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[14px] font-medium ${isActive ? 'text-violet-600' : isDone ? 'text-violet-400' : 'text-gray-400'}`}>
              {label}
            </span>
          </div>
        </React.Fragment>
...
const Spinner: React.FC<{ text: string; step?: string }> = ({ text, step }) => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8">
      ...
      <p className="text-[14px] text-gray-500 animate-fade-in" key={msgIdx}>{messages[msgIdx]}</p>
    </div>
  );
};
...
const ActionBar: React.FC<{
  ...
  <div className="px-6 py-4 border-t border-gray-200/60 flex justify-between items-center">
    <button
      onClick={onBack}
      className="px-5 py-2.5 text-[14px] font-medium text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors"
    >
      {backLabel}
    </button>
    <button
      ...
      className={`px-6 py-2.5 text-[14px] font-semibold rounded-xl transition-all ${
        nextDisabled
        ...
        <div className="max-w-xl mx-auto space-y-4">
          <label className="block">
            ...
            <input
              ...
              className="mt-1.5 w-full px-4 py-2.5 text-[14px] bg-white border border-gray-300 rounded-xl focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 outline-none"
            />
          </label>

          <label className="block">
            <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Subtitle</span>
            <input
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="block">
              <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Main Color</span>
            ...
            <label className="block">
              <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Background</span>
              <div className="mt-1.5 flex items-center gap-3 px-3 py-2 bg-white border border-gray-300 rounded-xl">
                <input
                  type="color"
          ...
          <label className="block">
            <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Style Keywords</span>
            <input
  ...
  <>
    <div className="flex-1 overflow-y-auto px-6 py-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[15px] font-medium text-gray-700">Choose a background image</p>
          {keywordsUsed && keywordsUsed.length > 0 && (
            <p className="text-[13px] text-gray-500 mt-0.5">Keywords: {keywordsUsed.join(', ')}</p>
          )}
        </div>
        <button
          onClick={onRefresh}
          disabled={isRefreshing}
          className="flex items-center gap-1.5 text-[13px] text-violet-500 hover:text-violet-700 font-medium disabled:opacity-50 transition-colors"
        >
    ...
    >
      ...
      <div className="border-b border-gray-200/60">
        <div className="flex items-center justify-between px-6 pt-4 pb-0">
          <h2 className="text-lg font-semibold text-gray-900">Design Wizard</h2>
          <button
            onClick={onCancel}
            className="flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-medium text-gray-500 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
          >

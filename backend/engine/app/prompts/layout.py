...
- "top_text"       — 上文下图：标题左对齐在画布上方，大面积背景呼吸。适合风景、建筑、旅行类
- "centered"       — 极简居中：大留白，文字垂直居中，高级感。适合奢侈品、极简、艺术类
- "bottom_heavy"   — 底部聚集：内容紧凑在底部左对齐，上方全给背景。适合摄影、人物、美食类
- "left_aligned"   — 杂志通栏：左对齐紧凑排版，像杂志封面。适合资讯、活动、商务类
- "diagonal"       — 对角线冲击：标题左上 + CTA右下，视觉张力。适合运动、科技、游戏类
- "big_title"      — 大字报：超大标题（1.8x）占据视觉中心，强冲击力。适合促销、公告、活动类
- "split_vertical" — 上下分割：信息分两区，节奏清晰。适合对比、活动、教育类
...
3. 【文字颜色】
   ...
   - 如果使用了 add_overlay，overlay 上方的文字可以大胆用色
...
特别注意：如果反馈提到「文字可读性」或「颜色接近」，确保文字与背景有足够的对比度（可换色、加 overlay、或调整字号）。"""
